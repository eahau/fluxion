...
class FluxionCoreAutoConfiguration {
    ...
    fun workflowEngine(
        ...
        @Autowired(required = false) executionLogStore: ExecutionLogStore?
     ...
     *
     * Accepts an optional [SetRefResolver] so that "published function sets"
     * (DAGs exposed as SET_REF wf_function rows) are transparently rewritten
     * to SUB_WORKFLOW invocations via the shared SubWorkflowExecutor path.
     */
    @Bean
    fun dagExecutor(
        workflowEngine: WorkflowEngine,
        ...
        @Autowired(required = false) taskInterceptor: TaskInterceptor?,
        @Autowired(required = false) providerRegistry: com.fluxion.schema.api.SchemaDataProviderRegistry?,
        @Autowired(required = false) subWorkflowExecutor: com.fluxion.core.engine.control.SubWorkflowExecutor?,
        @Autowired(required = false) loopExecutor: com.fluxion.core.engine.control.LoopExecutor?,
        @Autowired(required = false) waitExecutor: com.fluxion.core.engine.control.WaitExecutor?,
        @Autowired(required = false) setRefResolver: SetRefResolver?
    ...
    fun executionLogStore(): ExecutionLogStore = object : ExecutionLogStore {
        ...
        override fun save(record: NodeExecutionRecord, meta: ExecutionMeta) {
            if (record.status.name == "FAILED") {
                ...
                log.info { "Node [${record.nodeId}] ${record.status} duration=${record.durationMs}ms" }
            }
        }
    }
