...
/**
 * [GrpcExternalFunctionTransport] 鏈湴闆嗘垚娴嬭瘯銆?
 *
 * 鍚姩涓€涓甫 Server Reflection 鐨勬湰鍦?gRPC 鏈嶅姟锛岄獙璇佸姩鎬佸彂鐜颁笌鍙嶅皠璋冪敤銆?
 */
class GrpcExternalFunctionTransportTest {

    private val transport = GrpcExternalFunctionTransport()
    ...
    fun `missing service returns config error`() {
        val request = request(config = ExternalFunctionConfig(protocol = "grpc", service = null, method = "SayHello"))
    ...
    fun `missing endpoint returns config error`() {
        val request = request(
            config = ExternalFunctionConfig(protocol = "grpc", service = "example.Greeter", method = "SayHello", endpoint = null)
        )
    ...
    private fun request(config: ExternalFunctionConfig, input: Any? = null): ExternalFunctionRequest =
