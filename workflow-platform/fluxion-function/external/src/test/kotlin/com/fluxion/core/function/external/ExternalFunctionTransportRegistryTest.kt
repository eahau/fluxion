...
class ExternalFunctionTransportRegistryTest {

    @Test
    fun `register and resolve transport`() {
        val registry = ExternalFunctionTransportRegistry(useServiceLoader = false)
    ...
    fun `loads transports from ServiceLoader`() {
        val registry = ExternalFunctionTransportRegistry(useServiceLoader = true)

        assertTrue(registry.contains("spi-mock"))
        val response = registry.resolve("spi-mock")!!.invoke(
            ExternalFunctionRequest(config = ExternalFunctionConfig(), input = null, functionRef = "test")
        )
    ...
    fun `prepare is skipped after shutdown`() {
        ...
        registry.prepare(listOf(ExternalFunctionConfig(protocol = "http")))
    ...
    private class MockTransport(val protocolName: String) : ExternalFunctionTransport {
        ...
        override fun invoke(request: ExternalFunctionRequest): ExternalFunctionResponse =
            ExternalFunctionResponse(output = null)
