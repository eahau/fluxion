 ...
 * [OutboundTransportLifecycle] on shutdown.
 */
 ...
 */
class GrpcOutboundTransport : OutboundTransport {
    ...
    override suspend fun invokeSuspend(request: OutboundRequest): OutboundResponse {
        ...
        return try {
            ...
            val responseMessage = performUnaryCallAsync(
                channel,
                methodDescriptor,
                requestMessage,
            ...
            OutboundResponse(output = convertMessage(responseMessage))
    ...
    private fun buildChannel(config: OutboundConfig): ManagedChannel {
    ...
    private fun buildChannelKey(config: OutboundConfig): String {
        val useTls = config.extras?.get("useTls")?.toString()?.toBooleanStrictOrNull() ?: false
    ...
    private suspend fun performUnaryCallAsync(
        channel: Channel,
        methodDescriptor: Descriptors.MethodDescriptor,
        request: DynamicMessage,
        metadata: Metadata,
        ...
        val interceptedChannel = ClientInterceptors.intercept(channel, MetadataInterceptor(metadata))
        val call = interceptedChannel.newCall(grpcMethod, CallOptions.DEFAULT.withDeadlineAfter(timeoutMs, TimeUnit.MILLISECONDS))
        val listenableFuture = ClientCalls.futureUnaryCall(call, request)
        val completableFuture = CompletableFuture<DynamicMessage>()
