 ...
 * [OutboundTransport] implementations.
 *
 ...
 *    `META-INF/services/...OutboundTransport`.
 *
 ...
 */
class OutboundTransportRegistry(
    transports: List<OutboundTransport> = emptyList(),
    useServiceLoader: Boolean = true
) {

    private val log = LoggerFactory.getLogger(OutboundTransportRegistry::class.java)

    private val registry: MutableMap<String, OutboundTransport> = ConcurrentHashMap()
    ...
    fun register(transport: OutboundTransport) {
        register(transport, source = "explicit")
    }

    private fun register(transport: OutboundTransport, source: String) {
        val protocol = transport.protocol().lowercase()
        val previous = registry.put(protocol, transport)
        if (previous != null) {
            log.warn {
                "OutboundTransport for protocol '$protocol' has been overridden by $source: " +
                    "${previous.javaClass.name} -> ${transport.javaClass.name}"
            }
        } else {
            log.info {
                "Registered OutboundTransport for protocol '$protocol' from $source: ${transport.javaClass.name}"
            }
        }
    }

    /** Removes the transport associated with `protocol`, if any. */
    fun unregister(protocol: String): OutboundTransport? {
        val key = protocol.lowercase()
        val removed = registry.remove(key)
        removed?.let { log.info { "Unregistered OutboundTransport for protocol '$key'" } }
        return removed
    }

    /** Resolves the transport responsible for a given protocol name. */
    fun resolve(protocol: String?): OutboundTransport? {
    ...
    fun shutdown() {
        if (!shutdown.compareAndSet(false, true)) {
            log.debug { "OutboundTransportRegistry already shutdown, skipping" }
            return
        }
        ...
        log.info { "OutboundTransportRegistry shutdown completed" }
    }
    ...
    companion object {
        ...
        @JvmStatic
        fun loadFromServiceLoader(): List<OutboundTransport> {
            return ServiceLoader.load(OutboundTransport::class.java).toList()
        }
    }
}