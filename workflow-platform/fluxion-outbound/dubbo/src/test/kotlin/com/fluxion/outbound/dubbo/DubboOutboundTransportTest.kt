 ...
 */
class DubboOutboundTransportTest {
    private val transport = DubboOutboundTransport()
    ...
    fun `missing service returns config error`() {
        val request = request(config = OutboundConfig(protocol = "dubbo", service = null, method = "sayHello"))
    ...
    fun `missing method returns config error`() {
        val request = request(config = OutboundConfig(protocol = "dubbo", service = DemoService::class.java.name, method = null))
        val response = transport.invoke(request)
        assertThat(response.success).isFalse()
        assertThat(response.errorCode).isEqualTo("DUBBO_CONFIG_ERROR")
    ...
    private fun exportService(impl: DemoService) {
        ...
        serviceConfig.application = ApplicationConfig("fluxion-outbound-test-provider")
