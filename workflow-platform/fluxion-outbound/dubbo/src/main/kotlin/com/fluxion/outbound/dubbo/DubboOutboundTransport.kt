 ...
 * delegated to [OutboundTransportLifecycle] so we never leak a
 * Dubbo scope model across redeploys.
 */
package com.fluxion.outbound.dubbo
...
import com.fluxion.outbound.OutboundTransportLifecycle
 ...
 */
class DubboOutboundTransport : OutboundTransport {

    companion object {
        private const val DEFAULT_APPLICATION_NAME = "fluxion-outbound"
    }
    ...
    override suspend fun invokeSuspend(request: OutboundRequest): OutboundResponse {
        ...
        return try {
            ...
            val result = genericService.`$invokeAsync`(method, parameterTypes, args).await()
            OutboundResponse(output = convertResult(result))
        } catch (ex: Exception) {
    ...
    private fun buildReference(config: OutboundConfig): ReferenceConfig<GenericService> {
    ...
    private fun buildCacheKey(config: OutboundConfig): String {
        val applicationName = config.extraString("applicationName") ?: DEFAULT_APPLICATION_NAME
        val registryAddress = config.extraString("registryAddress") ?: ""
    ...
    private fun resolveParameterTypes(config: OutboundConfig, input: Any?): Array<String> {
        return config.extraStringList("parameterTypes")?.toTypedArray()
            ?: (input as? Map<*, *>)?.let { arrayOf(Map::class.java.name) }
            ?: emptyArray()
    }

    private fun resolveArgs(config: OutboundConfig, input: Any?): Array<Any?> {
