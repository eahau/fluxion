...
/**
 * gRPC 外部函数 Transport：通过 gRPC Server Reflection 动态发现服务并反射调用。
 *
 * 配置示例：
 * ```json
 * {
 ...
 * ```
 *
 * 目标 gRPC 服务必须开启 Server Reflection（通常由 `grpc-services` 提供）。
 */
class GrpcExternalFunctionTransport : ExternalFunctionTransport {
    ...
    override fun invoke(request: ExternalFunctionRequest): ExternalFunctionResponse {
        ...
        return try {
            ...
            log.error("gRPC invoke failed: [$fullMethodName] -> ${ex.status}", ex)
            ...
            log.error("gRPC invoke failed: [$fullMethodName]", ex)
            ExternalFunctionResponse(
                output = null,
                success = false,
    ...
    private fun buildFileDescriptor(protoList: List<DescriptorProtos.FileDescriptorProto>): Descriptors.FileDescriptor {
        ...
        // 构建所有 proto，返回包含主服务定义的那个（通常最后一个）
