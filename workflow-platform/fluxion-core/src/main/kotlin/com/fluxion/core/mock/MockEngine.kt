...
/**
 * 工作流调试 Mock 引擎（基于 AviatorScript）。
 *
 * 对外三个公开 API 与原 JEXL 版本 / MockEngineTest 完全对齐：
 *   - [evaluate]          根据 MockConfig + functionRef + NodeInput 找命中规则并返回 mock 结果
 *   - [renderTemplate]    独立把响应模板渲染为 JSON 字符串（`${var}` / `${var:default}` / `#{aviatorExpr}`）
 *   - [flattenNodeInput] 把 NodeInput 展平成 `directInput.userId -> "u001"` 形式，供 FieldMatcher 使用
 */
object MockEngine {

  private val log = LoggerFactory.getLogger(javaClass)

  /** `${var}` 或 `${var:default}` 形式（变量占位，带可选默认值） */
  private val templateVarPattern = Pattern.compile("\\$\\{([^:}]+)(?::([^}]*))?}")

  /** `#{aviator-expr}` 形式（完整表达式求值） */
  private val templateExprPattern = Pattern.compile("#\\{([^}]+)}")

  /** SpEL `#identifier` 前缀剥除 */
  ...
  /**
   * 辅助：构造一个可变参数的 AviatorFunction 匿名实例，把 AviatorObject[] 全部转为 Java 类型后转发给 body。
   */
  ...
  /**
   * Aviator 单例（使用 getInstance() 共享已加载内置模块的实例）+ 沙箱选项 + fn.* 方法注册。
   * 注意：newInstance() 创建的空白实例不会自动加载 string/math/seq/date/regexp 等内置函数库；
   *       getInstance() 已通过 ServiceLoader 扫描 META-INF/aviator_functions 完成内置模块注册。
   */
  private val aviator: AviatorEvaluatorInstance = AviatorEvaluator.getInstance().also { inst ->
    ...
    // 5 个 fn.* 工具函数（业务专属 + 模拟强需求；字符串/集合/随机浮点工具直接用 Aviator 内置）
  ...
  // ============================================================
  // 公开 API（MockEngineTest 1:1 对齐）
  // ============================================================

  /**
   * 主入口：根据 functionRef 找命中规则，返回 mock 结果，或 null（没命中）。
   *
   * 返回值：
   *   - config.enabled == false → null
   *   - 所有规则都 disabled / functionRef 不匹配 / 条件不命中 → null
   *   - 命中规则且 `response != null` → response 深拷贝（防止外部修改污染）
   *   - 命中规则且只有 `responseTemplate` → 渲染后解析为 JSON 对象（失败则抛给调用方）
   */
  fun evaluate(config: MockConfig, functionRef: String, nodeInput: NodeInput): Any? {
    ...
    for (rule in rules) {
      if (conditionHit(rule.condition, nodeInput)) {
        // 命中：返回静态 response > responseTemplate 解析后的 JSON
        ...
        return when {
          ...
          rule.responseTemplate != null -> {
            ...
            // 模板一般是 JSON 字符串；这里解析回 Map/List/基本类型
  ...
  /**
   * 独立模板渲染：`${var:default}` 占位 + `#{expr}` 表达式。
   *
   * MockEngineTest 中直接调用：`renderTemplate("""{"ts":#{fn.now()}...}""", nodeInput())` 期望返回 JSON 字符串。
   */
  ...
  /**
   * 把 NodeInput 展平为 key=string, value=string 的 Map，供 FieldMatcher 使用。
   *
   * 输出示例：
  ...
  private fun conditionHit(condition: MockCondition?, nodeInput: NodeInput): Boolean {
    ...
    val exprOk = condition.expression.isNullOrBlank() || runCatching {
      ...
      log.warn("Mock 表达式条件失败: expr={}, error={}", condition.expression, e.message, e)
      false
    }
    if (!exprOk) return false
    // 2. fieldMatchers：展平后全匹配
  ...
  /**
   * 构造 Aviator env（与 JEXL 旧版对齐）：
   *   - directInput / workflowInput / declaredDeps / nodeParams 四个语义域
   *   - 顶层展平优先级：directInput > workflowInput > declaredDeps > nodeParams（与原 JEXL 版一致）
   */
  ...
  private fun resolvePath(path: String, env: Map<String, Any?>): Any? {
    ...
    for (i in 1 until parts.size) {
      ...
      current = when (current) {
        is Map<*, *> -> current[part]
        is List<*> -> part.toIntOrNull()?.let { idx -> if (idx in current.indices) current[idx] else null }
        else -> null
      }
    }
    return current
  ...
  /** Aviator 真值判断：与 ExpressionEvaluator.evalBoolean 规则一致 */
  ...
  /** 展平任意嵌套结构为 `prefix.key -> value.toString()` 的 Map（FieldMatcher 用） */
  ...
  /** 深拷贝：避免命中静态 response 返回的 Map/List 被外部修改而污染 config */
  ...
  /** 解析 JSON 字符串到对象；失败返回 null（外部 fallback 为原字符串） */
  private fun parseJsonSafely(s: String): Any? = try {
    // Jackson 在此模块未作为依赖；但 responseTemplate 渲染后一般是 JSON，用一个简单的解析。
    // 这里用最小可用方案：尝试用 Aviator 内置不行；改为手动解析太复杂。
    // 由于 fluxion-schema:json 是 api 依赖、里面有 jackson（fluxion-core 的 api 依赖），可复用其 ObjectMapper。
    // 采用 java/serviceLoader 找不到就退一步：直接返回 null 让上层返回 rendered string。
...
/**
 * Mock 模板 fn.* 工具函数（通过 AbstractVariadicFunction 匿名实现注册为 AviatorScript 全局函数）。
 *
 * 仅保留业务语义无法被 Aviator 原生覆盖的部分；字符串/集合/浮点随机等通用工具直接用：
 ...
 *   - count(x) / include(col,x) / rand() 等顶层内置函数
 */
object FnFunctions {
  @JvmStatic
...
/**
 * 包级便捷函数（保持与原 JEXL 版本的调用点兼容：
 *   MockWorkflowFunction 直接调用 `evaluateMock(config, ref, input)` 而非 `MockEngine.evaluate`）。
 */
@Suppress("unused")
