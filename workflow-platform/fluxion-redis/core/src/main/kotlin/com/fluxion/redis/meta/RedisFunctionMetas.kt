...
object RedisFunctionMetas {
    ...
    // ─────────────────────────────────────────────────────────────────────
    // Tag alphabet (keys point to the representative signature; commands map 1:1):
    //
    //  Tag  | Meaning                         | Example commands
    //  -----+---------------------------------+------------------------------------
    //  K    | key only                        | GET, TTL, LLEN
    //  KV   | key + value [value ...]         | SET, LPUSH, SADD
    //  K_FV | key + field value pairs         | HSET, HMSET
    //  K_F  | key + field                     | HGET, HEXISTS
    //  K_SM | key + score member pairs        | ZADD
    //  K_R  | key + start/stop range         | LRANGE, ZRANGE
    //  K_N  | key + number                    | EXPIRE, PEXPIRE
    //  K_T  | key + timestamp                 | EXPIREAT
    //  P    | pattern (glob/regex)            | KEYS
    //  D    | destination + source keys       | SDIFFSTORE, ZUNIONSTORE
    //  S    | script + keys + args            | EVAL
    //  N    | no args                         | DBSIZE, FLUSHALL, PING
    //  X    | nested sub-command list         | PIPELINE
    // ─────────────────────────────────────────────────────────────────────
    ...
    /** command name → UI category tag, used by the frontend to group the dropdown. */
    ...
    /**
     * Builds the `x-i18n` metadata extension, carrying per-locale `description` values
     * so the frontend can display bilingual hints without duplicating schema trees.
     */
    private fun i18n(zh: String, en: String) = mapOf(
        "x-i18n" to mapOf("zh" to mapOf("description" to zh), "en" to mapOf("description" to en))
    )

    /** Shared `$ref`-capable schema used for `key`, `args` item, and `keys` item. */
    private val BINDING = mapOf("type" to listOf("string", "object"), "x-widget" to "binding")

    /**
     * Top-level `x-commands` metadata carrying the expanded `{ args, category }`
     * descriptor per Redis command. The frontend re-reads this map whenever the
     * selected command changes and toggles visibility of `args`, `keys`, `script`,
    ...
    /**
     * Reusable property shape for one Redis command (command + key + args tuple).
     * Shared between the top-level function schema and the nested items schema
     * of the PIPELINE `commands` array.
     */
    private val COMMAND_ITEM_PROPS = mapOf(
        ...
        ) + i18n("Command argument list (structured binding supported)",
    ...
    private val schemaJson = JsonUtil.serialize(
        mapOf(
            "type" to "object",
            // Raw vs structured parameters are intentionally not mutually exclusive at
            // schema layer; the runtime function picks `raw` first when present.
            ...
            "properties" to (COMMAND_ITEM_PROPS + mapOf(
                "raw" to mapOf(
                    ...
                    "Raw Redis CLI line (e.g. SET user:1 name John; \${var} interpolation)",
                ...
                "script" to mapOf("type" to "string", "description" to "Lua script body (for EVAL command only)")
                    + i18n("Lua script body (EVAL command only)", "Lua script (EVAL command only)"),
                ...
                ) + i18n("PIPELINE sub-command list", "PIPELINE command list"),
            )),
        )
    )
    ...
    val REDIS_COMMAND = FunctionMeta.builder("builtin:redisCommand")
        ...
        .descriptions(mapOf(
            "zh" to "通用 Redis 命令执行器，支持所有数据结构和命令（通过 RedisClientAdapter SPI 解耦具体客户端）",
