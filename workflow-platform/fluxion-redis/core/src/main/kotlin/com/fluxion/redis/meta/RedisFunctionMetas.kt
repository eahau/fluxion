 ...
 * 内置函数元信息常量。
 *
 * 所有 Redis 命令的参数签名来自 [Redis 官方文档](https://redis.io/commands/)，
 * 通过声明式定义自动生成 paramSchema（enum / x-commands / x-widget 等），前端零硬编码。
 */
object RedisFunctionMetas {

    // ─── 命令参数签名（基于 Redis CLI 语法）───────────────────────────
    //
    //  签名  │ 含义                     │ 代表命令
    //  ─────┼─────────────────────────┼──────────────────
    //  K    │ key                      │ GET, TTL, LLEN
    //  KV   │ key + value [value ...]  │ SET, LPUSH, SADD
    //  K_FV │ key + field value [...]  │ HSET, HMSET
    //  K_F  │ key + field              │ HGET, HEXISTS
    //  K_SM │ key + score member [...] │ ZADD
    //  K_R  │ key + start stop         │ LRANGE, ZRANGE
    //  K_N  │ key + number             │ EXPIRE, PEXPIRE
    //  K_T  │ key + timestamp          │ EXPIREAT
    //  P    │ pattern                  │ KEYS
    //  D    │ destination + keys       │ SDIFFSTORE, ZUNIONSTORE
    //  S    │ script + keys + args     │ EVAL
    //  N    │ 无参数                    │ DBSIZE, FLUSHALL, PING
    //  X    │ pipeline 子命令列表       │ PIPELINE
    ...
    /**
     * 多语言描述扩展：为 schema 字段注入 `x-i18n` 元数据。
     * 前端根据当前 locale 提取对应 description，实现零硬编码国际化。
     */
    private fun i18n(zh: String, en: String) = mapOf(
        "x-i18n" to mapOf("zh" to mapOf("description" to zh), "en" to mapOf("description" to en))
    )

    /** 支持 $ref 绑定的字段 schema（key / args item / keys item 复用） */
    private val BINDING = mapOf("type" to listOf("string", "object"), "x-widget" to "binding")

    /** x-commands 元数据：{ commandName: { args, category } }，前端据此动态显隐字段 */
    private val xCommands = COMMANDS.associate { (name, args) ->
        name to mapOf("args" to args, "category" to (categoryOf[name] ?: "special"))
    }

    /** 单条 Redis 命令的属性定义（command / key / args 三要素），顶层和 PIPELINE 子项共用 */
    private val COMMAND_ITEM_PROPS = mapOf(
        "command" to mapOf(
            "type" to "string", "description" to "Redis 命令",
            "enum" to COMMANDS.map { it.first },
            "x-commands" to xCommands,
            "x-widget" to "searchableSelect",
        ) + i18n("Redis 命令", "Redis Command"),
        "key" to BINDING + mapOf("description" to "Redis Key（支持 \${var} 模板插值或结构化绑定）")
            + i18n("Redis Key（支持 \${var} 模板插值或结构化绑定）", "Redis Key (supports \${var} template interpolation or structured binding)"),
        "args" to mapOf(
            "type" to "array", "items" to BINDING,
            "description" to "命令参数列表（支持结构化绑定）",
        ) + i18n("命令参数列表（支持结构化绑定）", "Command arguments (supports structured binding)"),
    )

    private val schemaJson = JsonUtil.serialize(
        mapOf(
            "type" to "object",
            // raw 与结构化参数（command/key/args）二选一，不在 schema 层强制 required
            "required" to emptyList<String>(),
            "properties" to (COMMAND_ITEM_PROPS + mapOf(
                "raw" to mapOf(
                    "type" to "string",
                    "description" to "原始 Redis CLI 命令行（如 SET user:1 name John；支持 \${var} 模板插值）",
                    "x-widget" to "textarea",
                ) + i18n(
                    "原始 Redis CLI 命令行（如 SET user:1 name John；支持 \${var} 模板插值）",
                    "Raw Redis CLI command line (e.g. SET user:1 name John; supports \${var} template interpolation)"
                ),
                "script" to mapOf("type" to "string", "description" to "Lua 脚本（EVAL 命令专用）")
                    + i18n("Lua 脚本（EVAL 命令专用）", "Lua script (EVAL command only)"),
                "keys" to mapOf("type" to "array", "items" to BINDING, "description" to "EVAL KEYS 参数")
                    + i18n("EVAL KEYS 参数", "EVAL KEYS arguments"),
                "commands" to mapOf(
                    "type" to "array",
                    "items" to mapOf("type" to "object", "properties" to COMMAND_ITEM_PROPS),
                    "description" to "PIPELINE 命令列表",
                ) + i18n("PIPELINE 命令列表", "PIPELINE command list"),
            )),
        )
    )
    ...
    val REDIS_COMMAND = FunctionMeta.builder("builtin:redisCommand")
        .description("通用 Redis 命令执行器，支持所有数据结构和命令（通过 RedisClientAdapter SPI 解耦客户端）")
        .descriptions(mapOf(
            "zh" to "通用 Redis 命令执行器，支持所有数据结构和命令（通过 RedisClientAdapter SPI 解耦客户端）",
